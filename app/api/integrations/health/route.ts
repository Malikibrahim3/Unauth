...
export async function GET() {
  ...
  const connections = ((connectionResult.data ?? []) as ConnectionRow[]).map((row) => ({ ...row, freshness: connectionFreshness(row.data_fresh_through ?? row.last_successful_sync_at, now), recordCounts: records.filter((record) => record.connection_id === row.id).reduce((counts: Record<string, number>, record) => ({ ...counts, [record.source_entity_type]: (counts[record.source_entity_type] ?? 0) + 1 }), {}), activeIssueCount: issues.filter((issue) => issue.connection_id === row.id).length }));
  return NextResponse.json({ connections, coverage: coverageFromRecords(records), issues });
}
