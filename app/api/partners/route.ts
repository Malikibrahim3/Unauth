...
import { createPartner, listPartners, type CreatePartnerInput } from '@/lib/partners/store';
import type { Permission } from '@/lib/permissions';

export const dynamic = 'force-dynamic';

// Reads need view; create/update/deactivate need manage (a view-only user must
// not be able to write to the partner rulebook).
...
export async function GET() {
  ...
  const partners = await listPartners(auth.serviceClient, auth.ctx.merchantId);
...
export async function POST(request: Request) {
  ...
  try {
    const partner = await createPartner(auth.serviceClient, {
