...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const parsedConditions = parsed.data.conditions as
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
  ...
  const { data, error } = await (serviceClient as any).rpc(
    ...
    { p_merchant_id: ctx.merchantId, p_actor_id: user.id, p_rule_id: id },
  );
  if (error) {
    const missing = error.message.includes("rule_not_found");
    return NextResponse.json(
      { error: missing ? "Rule not found" : "Failed to archive rule" },
