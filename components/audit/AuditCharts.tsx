  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <SectionCard title="Match distribution" description={`${totalTiered.toLocaleString()} ${totalTiered === 1 ? 'customer' : 'customers'} with match signals`}>
        {totalTiered === 0 ? (
          <div className="flex items-center justify-center h-[120px] rounded-md" style={{ background: 'var(--surface)' }}>
            <div className="text-center">
              <div
                className="inline-flex items-center justify-center w-10 h-10 rounded-full mb-2"
                style={{ background: 'var(--sev-clear-fill)', color: 'var(--neutral)' }}
              >
                <Check className="w-5 h-5" aria-hidden="true" />
              </div>
              <p className="text-caption font-medium" style={{ color: 'var(--text-primary)' }}>No match signals found</p>
              <p className="text-caption" style={{ color: 'var(--text-tertiary)' }}>All transactions appear clean</p>
            </div>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {/* Stacked horizontal bar */}
            <div className="h-5 w-full rounded-sm overflow-hidden flex" style={{ background: 'var(--surface-sunken)' }}>
              {TIERS.map((t) => {
            ...
            <div className="flex flex-col gap-1.5">
              {TIERS.flatMap((t) => (counts[t.key] > 0 ? [(
                <div key={t.key} className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-mono" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>
          ...
          {TIERS.map((tier) => {
            ...
            return (
              <div key={tier.key} className="flex h-full min-w-0 flex-col justify-end gap-2">
                <div className="flex h-32 items-end rounded-sm" style={{ background: 'var(--surface-sunken)', border: '1px solid var(--border)' }}>
                  <div
                ...
                <div className="text-center">
                  <p className="text-caption font-mono" style={{ color: 'var(--text-primary)' }}>{value.toLocaleString()}</p>
                  <p className="text-xs truncate" style={{ color: 'var(--text-secondary)' }}>{tier.label}</p>
                </div>
              </div>
            );
          })}
      ...
      <SectionCard title="Overall composition" className="md:col-span-2">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-4 text-caption" style={{ color: 'var(--text-tertiary)' }}>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-sm inline-block" style={{ background: 'var(--accent)' }} />
              With signals {flaggedPct.toFixed(1)}%
            </span>
            <span className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-sm inline-block" style={{ background: 'var(--neutral)' }} />
              No signals {cleanPct.toFixed(1)}%
            </span>
          </div>
        </div>
        <div className="h-2 w-full rounded-sm overflow-hidden" style={{ background: 'var(--surface-sunken)' }}>
          <div
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <span className="text-caption font-mono" style={{ color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}>
