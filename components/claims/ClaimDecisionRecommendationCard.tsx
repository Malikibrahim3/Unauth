...
export type ClaimDecisionPayload = {
  ...
  evaluation: {
    ...
    matched_conditions: Array<{ field: string; operator: string; value: unknown; actual_value: unknown }>;
    ...
    >
      ...
      {claimId && formatted && !loading && !error && (
        <div className="space-y-3">
            ...
            <div>
              ...
              <ul className="space-y-1">
                {formatted.matchedConditions.map((line) => (
                  <li key={`${line.label}-${line.actual}`} className="text-caption flex gap-1.5" style={{ color: 'var(--text-primary)' }}>
          ...
          <p className="text-caption leading-relaxed pt-1 border-t" style={{ color: 'var(--text-tertiary)', borderColor: 'var(--border-subtle)' }}>
            Unauth applies your rules to the available claim context. Your team makes the final decision.
          </p>
        </div>
      )}
    </RailSection>
