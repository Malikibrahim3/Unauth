    ...
    <div className="space-y-4 min-w-0 order-1 min-[1100px]:col-start-1 min-[1100px]:row-start-1">
        ...
        {withinStoreSignals.length === 0 ? (
          ...
          <div className="space-y-2">
            {withinStoreSignals.map((row) => (
              <div key={row.key} className="grid grid-cols-1 md:grid-cols-[1fr_1fr_1fr_auto] gap-2 rounded-md border p-2.5 text-xs" style={{ borderColor: 'var(--border-subtle)', background: 'var(--bg-inset)' }}>
                ...
                <span className="inline-flex items-center gap-2">
                  ...
                  <span className="rounded-full px-2 py-0.5 text-xs font-semibold" style={{ background: 'var(--bg-subtle)', color: 'var(--text-muted)' }}>{row.grade}</span>
                </span>
              </div>
            ))}
          </div>
      ...
      <section className="rounded-xl p-4 border" style={{ borderColor: 'var(--border-subtle)', background: 'var(--bg-surface)' }}>
        <div className="mb-3 inline-flex rounded-md border p-0.5" style={{ borderColor: 'var(--border-subtle)', background: 'var(--bg-inset)' }}>
          <button type="button" onClick={() => patch({ auditTab: 'timeline' })} className="px-2.5 py-1 text-xs rounded" style={{ background: state.auditTab === 'timeline' ? 'var(--accent)' : 'transparent', color: state.auditTab === 'timeline' ? 'var(--text-inverse)' : 'var(--text-muted)' }}>Event timeline</button>
          <button type="button" onClick={() => patch({ auditTab: 'history' })} className="px-2.5 py-1 text-xs rounded" style={{ background: state.auditTab === 'history' ? 'var(--accent)' : 'transparent', color: state.auditTab === 'history' ? 'var(--text-inverse)' : 'var(--text-muted)' }}>Claim history</button>
        </div>
