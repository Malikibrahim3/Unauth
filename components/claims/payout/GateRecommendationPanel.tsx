  ...
  if (!recommendation) {
    return (
      <Card unstyled as="section" variant="panel" className="p-4">
        <h3 className="ua-text-working-title" style={{ color: 'var(--uo-route-text-primary)' }}>Recommendation</h3>
        ...
        <Link href="/controls/rules" className="ua-text-label mt-2 inline-block underline underline-offset-2" style={{ color: 'var(--uo-route-text-primary)' }}>
          Review rules
        </Link>
      </Card>
    );
  ...
  return (
    <Card unstyled as="section" variant="panel" className="space-y-3 p-4">
      <div className="flex items-center justify-between gap-2">
        <h3 className="ua-text-working-title" style={{ color: 'var(--uo-route-text-primary)' }}>
          Review gate
        </h3>
      ...
      <div>
        {recommendation.reasoning.triggered_rules.length > 0 ? (
          <>
            <p className="text-caption font-medium mb-1.5" style={{ color: 'var(--uo-route-text-secondary)' }}>
              Why:
            </p>
            <ul className="space-y-1.5">
              {recommendation.reasoning.triggered_rules.map((rule) => (
                <li key={rule.rule_name} className="text-caption" style={{ color: 'var(--uo-route-text-primary)' }}>
                    ...
                    <ul className="mt-1 space-y-0.5 pl-3">
                      {rule.conditions_met.map((condition) => (
                        <li key={condition} className="flex gap-1.5">
                          <span aria-hidden style={{ color: 'var(--uo-route-text-tertiary)' }}>
                            •
                          </span>
                          <span>{condition}</span>
                        </li>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-caption" style={{ color: 'var(--uo-route-text-secondary)' }}>
          Evidence:
        </span>
      ...
      <div>
        <p className="text-caption font-medium mb-1" style={{ color: 'var(--uo-route-text-secondary)' }}>
          Recovery available:
        </p>
        {availableRoutes.length > 0 ? (
          <ul className="space-y-1">
            {availableRoutes.map((route) => (
              <li key={route.route} className="text-caption flex gap-1.5" style={{ color: 'var(--uo-route-text-primary)' }}>
                <span aria-hidden style={{ color: 'var(--uo-route-text-tertiary)' }}>
                  •
                </span>
      ...
      <div className="pt-2 border-t" style={{ borderColor: 'var(--uo-route-border-subtle)' }}>
        <p className="text-caption" style={{ color: 'var(--uo-route-text-primary)' }}>
          <span className="font-medium" style={{ color: 'var(--uo-route-text-secondary)' }}>
            Suggested next step:{' '}
          </span>
      ...
      {recommendation.limitations.length > 0 && (
        <ul className="space-y-1 pt-1">
          {recommendation.limitations.map((limitation) => (
            <li key={limitation} className="text-caption" style={{ color: 'var(--uo-route-text-tertiary)' }}>
              Note: {limitation}
            </li>
          ))}
        </ul>
