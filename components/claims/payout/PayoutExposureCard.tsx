  ...
  return (
    ...
    >
      ...
      {hasAmount ? (
        <>
          <div className="flex flex-wrap items-end gap-x-6 gap-y-2">
            <div>
              <p className="ua-text-label mb-0.5" style={{ color: 'var(--uo-route-text-secondary)' }}>
                Estimated value at issue
              </p>
              <p className="font-sans tabular-nums font-semibold" style={{ fontSize: 28, letterSpacing: '-0.02em', color: 'var(--uo-route-text-primary)' }}>
          ...
          {exposure.components.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-1.5">
              {exposure.components.map((c) => (
                ...
                >
                  <span style={{ color: 'var(--uo-route-text-primary)' }}>{COMPONENT_LABELS[c.kind] ?? c.kind}</span>
      ...
      <p className="ua-text-caption-role mt-3">
        Customer requested: <span style={{ color: 'var(--uo-route-text-primary)' }}>{requestedActionLabel}</span>
      </p>
    </section>
  );
}
