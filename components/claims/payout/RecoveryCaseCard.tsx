...
export function RecoveryCaseCard({
  ...
  const preventionOnly =
    recovery?.recoverability === 'not_recoverable' || recovery?.likelyOwner === 'merchant';
  const externalOwner =
    recovery != null &&
  ...
  return (
    <PanelCard as="section" variant="app" className="p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Button type="button" variant="secondary" size="sm" onClick={onRefresh} loading={loading} leadingIcon={<RefreshCw />}>
          Check route
        </Button>
      </div>
      ...
      {recoveryCase ? (
        <>
          <div className="mt-4 grid grid-cols-2 gap-3 text-sm md:grid-cols-4">
            <div>
              ...
              <p className="font-semibold" style={{ color: 'var(--text-primary)' }}>{RECOVERY_STATUS_LABELS[recoveryCase.status]}</p>
            </div>
            <div>
              <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Owner</p>
              <p className="font-semibold" style={{ color: 'var(--text-primary)' }}>{RECOVERY_OWNER_LABELS[recoveryCase.owner_type]}</p>
            </div>
            <div>
              <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Recoverable</p>
              <p className="font-mono" style={{ color: 'var(--text-primary)' }}>
                {formatCurrencyNullable(recoveryCase.estimated_recoverable_max, recoveryCase.currency) ?? '-'}
              </p>
            </div>
            <div>
              <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>Deadline</p>
              <p style={{ color: 'var(--text-primary)' }}>{dateLabel(recoveryCase.deadline_at)}</p>
            </div>
          </div>
          ...
          <Link href="/recoveries" className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold hover:underline" style={{ color: 'var(--accent)' }}>
            Open recovery board <ExternalLink className="h-3.5 w-3.5" aria-hidden="true" />
          </Link>
        </>
        ...
        <div className="mt-4 space-y-3 text-sm">
          <div>
            ...
            <p className="mt-1 text-xs" style={{ color: 'var(--text-secondary)' }}>
              {RECOVERY_OWNER_LABELS[recovery.likelyOwner as keyof typeof RECOVERY_OWNER_LABELS] ?? recovery.likelyOwner}
              {' · '}
          ...
          <p className="text-xs" style={{ color: 'var(--text-tertiary)' }}>
            Use “Check route” to open the recovery case and add it to the recovery board.
          </p>
        </div>
      ) : preventionOnly ? (
        <p className="mt-4 text-sm" style={{ color: 'var(--text-secondary)' }}>
          Prevention opportunity: this loss appears unrecoverable but can inform future policy or partner review.
        </p>
      ) : (
        <p className="mt-4 text-sm" style={{ color: 'var(--text-secondary)' }}>
