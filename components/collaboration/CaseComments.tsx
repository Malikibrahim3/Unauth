...
export function CaseComments({ caseId, canComment }: { caseId: string; canComment: boolean }) {
  ...
  return <section className="order-4 min-w-0 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4 min-[1100px]:col-start-1" aria-label="Case comments">
    ...
    <div className="mt-3 space-y-2">
      {comments.length ? comments.map((comment) => <article key={comment.id} className="rounded-md p-3 text-sm" style={{ background: 'var(--surface-muted, #f7f7f6)' }}>
        ...
        <p className="mt-1 text-xs" style={{ color: 'var(--text-tertiary)' }}>{formatDateTime(comment.created_at)}</p>
    ...
    {canComment ? <form onSubmit={submit} className="mt-4 space-y-3">
      ...
      <button type="submit" disabled={saving || !body.trim()} className="rounded-md px-3 py-1.5 text-sm font-medium" style={{ background: 'var(--accent)', color: 'var(--accent-fg-on-500)' }}>{saving ? 'Posting…' : 'Post comment'}</button>
    </form> : null}
