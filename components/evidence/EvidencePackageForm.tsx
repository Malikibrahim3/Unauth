...
export function EvidencePackageForm({
  ...
  const prevPreselectedOrderIdRef = useRef(preselectedOrderId);

  if (preselectedOrderId !== prevPreselectedOrderIdRef.current) {
    prevPreselectedOrderIdRef.current = preselectedOrderId;
    setUserOrderId('');
  }

  const { data: ordersData, loading: loadingOrders } = useFetchJson<OrdersResponse>(
    `/api/customers/${profileId}/orders`,
    {
  ...
  const { data: ce3Data, loading: priorMatchChecking } = useFetchJson<Ce3CheckResponse>(
    selectedOrderId
