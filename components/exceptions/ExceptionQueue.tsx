...
export function ExceptionQueue({ compact = false }: { compact?: boolean }) {
  ...
  if (loading) return <p className="text-sm" style={{ color: 'var(--ua-text-tertiary)' }}>Loading exception queue…</p>;
  if (!rows.length) return <p className="rounded-lg border p-6 text-center text-sm" style={{ color: 'var(--ua-text-tertiary)' }}>No open exceptions. Connected sources and automation are up to date.</p>;
  const visible = compact ? rows.slice(0, 5) : rows;
  return <div className="space-y-3">{error ? <p role="alert" className="text-sm" style={{ color: 'var(--ua-critical)' }}>{error}</p> : null}{visible.map((row) => {
