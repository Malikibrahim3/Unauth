...
export function ExceptionQueue({ compact = false }: { compact?: boolean }) {
  ...
  })}{compact && rows.length > visible.length ? <Link href="/work?view=integration-exceptions" className="ua-text-label underline">View all {rows.length} exceptions</Link> : null}</div>;
}
