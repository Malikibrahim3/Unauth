  ...
  return (
    <aside className="ua-app-sidebar hidden h-full w-60 shrink-0 flex-col border-r border-[var(--border)] md:flex" aria-label="Main navigation loading">
      <div className="border-b border-[var(--border)] px-3 py-3">
        <UnauthLogo variant="mono-dark" size={28} />
        <div className="mt-2 flex h-9 items-center gap-2 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] px-2">
          <span className="h-6 w-6 rounded-[var(--radius-sm)] bg-[var(--surface-selected)]" aria-hidden="true" />
          <span className="truncate text-xs font-semibold text-[var(--text-secondary)]">{merchantName ?? "Workspace"}</span>
        </div>
      </div>
