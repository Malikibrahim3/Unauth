...
import { Button, InsetGroup, JoinedSection, Select, Surface, Textarea } from '@/components/ui';
    ...
    <Surface structure="working" aria-label="Agreement management">
      ...
      <JoinedSection className="p-4 sm:p-5">
        ...
        <form ref={formRef} onSubmit={uploadAgreement} className="mt-4 space-y-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="ua-text-body font-medium">Agreement type<Select name="agreement_type" required defaultValue="COURIER" className="mt-1 block">{AGREEMENT_TYPES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Select></label>
      ...
      {uploadState.status === 'success' ? <JoinedSection className="p-4 sm:p-5">
        ...
        <form onSubmit={approveTerms} className="mt-4 space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2"><label className="ua-text-body font-medium">Claim type<Select name="applies_to_claim_type" defaultValue="LOST_PARCEL" className="mt-1 block"><option value="LOST_PARCEL">Lost parcel</option><option value="ITEM_NOT_RECEIVED">Item not received</option><option value="DELIVERED_NOT_RECEIVED">Delivered not received</option><option value="DAMAGED_ITEM">Damaged item</option><option value="MISSING_ITEM">Missing item</option><option value="WRONG_ITEM">Wrong item</option><option value="DELAYED_DELIVERY">Delayed delivery</option><option value="RETURN_EXCEPTION">Return exception</option><option value="CHARGEBACK">Chargeback</option><option value="ANY">Any claim</option></Select></label><label className="ua-text-body font-medium">Rule effect<Select name="rule_type" defaultValue="RECOVERY_ELIGIBILITY" className="mt-1 block"><option value="RECOVERY_ELIGIBILITY">Recovery eligibility</option><option value="EVIDENCE_REQUIREMENT">Evidence requirement</option><option value="DEADLINE">Deadline</option><option value="LIABILITY_CAP">Liability cap</option><option value="EXCLUSION">Exclusion</option><option value="ESCALATION">Escalation</option></Select></label><label className="ua-text-body font-medium">Recovery status<Select name="recovery_eligible" defaultValue="eligible" className="mt-1 block"><option value="eligible">Eligible</option><option value="not_eligible">Not eligible</option><option value="pending_evidence">Pending evidence</option></Select></label></div>
          ...
          <label className="block ua-text-body font-medium">Required evidence<Input name="required_evidence" placeholder="Tracking scan, invoice" /></label><label className="block ua-text-body font-medium">Verified reason<Textarea name="reason" required maxLength={1000} rows={3} className="mt-1 block" placeholder="Describe the verified agreement term." /></label>
