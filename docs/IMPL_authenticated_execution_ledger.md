...
**Shell consistency (cross-cutting) — done 2026-08-02.** `WorkbenchPage`, `DetailPageShell`, and `SettingsPageShell` now all compose `PageFrame` internally instead of re-deriving the header + body wiring (the `metricStrip`/`ua-settings-form` content is passed through `PageFrame`'s `metrics`/`children` slots — DOM- and CSS-class-identical to before). The ~21 route files that hand-imported `AuthenticatedPageChrome.module.css` to hand-roll `AuthenticatedPageHeader` + `pageBody` are down to the small set that legitimately reuse body-only classes (`detailStack`/`panel`/skeletons that intentionally mirror real markup); the rest now render `PageFrame` (`CustomerProfilePageView`, `OperationalRouteError` — covers all ~24 `error.tsx` routes, `not-found.tsx`, the evidence/new and integrations routes). `rules/[id]` and `flows/[id]` render no header at all (their workbench owns its own toolbar), so `PageFrame`'s `title` was made optional — passing none skips `AuthenticatedPageHeader` entirely rather than rendering an empty one; every other consumer is unaffected. `/dashboard` and `/settings/billing` already compose real shells (`DashboardOverview`/`SettingsPageShell`) as of this pass — the "no shell at all" claim no longer holds. Verified via `typecheck`, `lint:authenticated-design`, and `verify-visual-adoption.mjs` (no regressions); browser visual check was not possible this session (another session's dev server holds the only port Next allows per checkout).
