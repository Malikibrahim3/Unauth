-- TARGETED-PATCH-ONLY — production lquvbikyvmbjbfffrlky, 2026-07-27
--
-- Composed mechanically from the approved migration files; SQL is not retyped.
-- Scope: the two migrations whose complete SQL is appliable on this database,
-- plus the applicable subset of the grants file.
--
-- EXCLUDED and why (verified by preflight, not assumed):
--   20260723200000_release1_investigations.sql
--     - requires public.capture_sensitive_audit_event() for two durable-audit
--       triggers; absent on production (created by 20260721120000, unapproved).
--     - requires the partners_id_merchant_id_key unique constraint for its
--       (partner_id, merchant_id) foreign key; absent on production (created by
--       20260723100000, unapproved). The ALTER would error outright.
--     Applying it minus the audit triggers would leave investigation tables with
--     no audit history, so it is skipped entirely rather than narrowed.
--   20260727130000_recovery_source_freshness.sql — not approved.
--   2 grants on case_prevention_observations — table absent (20260722400000).
--
-- One deliberate difference from the source files: a `drop trigger if exists`
-- guard before trg_work_saved_views_updated, the only non-idempotent statement
-- in 20260724110000. Effect is identical, so that file still counts as applied
-- in full.
...
-- ============================================================
-- 20260724110000_work_saved_views.sql (complete)
-- ============================================================
...
drop trigger if exists trg_work_saved_views_updated on public.work_saved_views;
create trigger trg_work_saved_views_updated
before update on public.work_saved_views
for each row execute function public.set_updated_at();

-- ============================================================
-- 20260725100000_evidence_reconciliation_pivot.sql (complete)
-- ============================================================
...
-- ============================================================
-- 20260727100000 grants — applicable subset (NARROWED, not recorded)
-- ============================================================
