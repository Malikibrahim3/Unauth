...
const s = StyleSheet.create({
  ...
  sectionLabel:  { fontSize: 12, fontFamily: 'Helvetica-Bold', color: C.accent, letterSpacing: 1, marginBottom: 6, marginTop: 14 },
  sectionSubhead:{ fontSize: 12, color: C.muted, marginBottom: 6 },
  narrative:     { fontSize: 12, color: C.text, lineHeight: 1.6 },
  ...
  tableCell:     { fontSize: 12, color: C.text },
  tableCellMuted:{ fontSize: 12, color: C.muted },
  ...
  highlightTitle:      { fontSize: 12, fontFamily: 'Helvetica-Bold', color: C.accent, marginBottom: 3 },
  highlightText:       { fontSize: 12, color: C.darkText, lineHeight: 1.5 },
  infoBox:       { backgroundColor: C.subtleBg, borderWidth: 1, borderColor: C.border, padding: 10, marginBottom: 8 },
  infoTitle:     { fontSize: 12, fontFamily: 'Helvetica-Bold', color: C.muted, marginBottom: 3 },
  ...
  amberText:     { fontSize: 12, color: C.darkText, lineHeight: 1.5 },
  assessRow:     { flexDirection: 'row', marginBottom: 4 },
  assessLabel:   { fontSize: 12, color: C.muted, width: 160 },
  assessValue:   { fontSize: 12, color: C.text, flex: 1 },
  assessValueGreen: { fontSize: 12, color: C.green, fontFamily: 'Helvetica-Bold', flex: 1 },
  ...
  noteItalic:    { fontSize: 12, color: C.muted, marginTop: 4 },
})
...
function PDFIdentityEvidenceTable({ pkg }: { pkg: EvidencePackage }) {
  const colWidths = IDENTITY_EVIDENCE_COL_WIDTHS
  return (
    <View style={s.table}>
      ...
      {pkg.identityEvidence.map((ev) => (
        <View key={`${ev.identifierType}-${ev.maskedValue}`} style={s.tableRow}>
...
function PDFOrderHistoryTable({ pkg }: { pkg: EvidencePackage }) {
  const colWidths = ORDER_HISTORY_COL_WIDTHS
  return (
    <View style={s.table}>
      <View style={s.tableHeader}>
      ...
      {pkg.orderHistory.map((order) => {
        ...
        return (
          <View key={order.orderId} style={rowStyle}>
    ...
    <View style={strength === 'Strong' ? s.highlightBox : s.infoBox}>
      ...
      {ce3.disqualifyingFactors.length > 0 && strength !== 'Strong' && (
        <View style={{ marginTop: 6 }}>
          {ce3.disqualifyingFactors.map((f) => (
            <Text key={f} style={s.infoText}>• {f}</Text>
          ))}
        </View>
      )}
    </View>
...
export default function EvidenceDocument({ pkg, narrative }: { pkg: EvidencePackage; narrative: string }) {
  return (
