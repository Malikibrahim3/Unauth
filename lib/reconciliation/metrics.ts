...
export type AutomationMetrics = {
  ...
  automationCompletionPercent: number; averageMerchantInputsPerCase: number; reconciliationLagHours: number | null; recoveryTasks: number;
};

export async function getAutomationMetrics(client: SupabaseClient, merchantId: string): Promise<AutomationMetrics> {
  const [casesResult, eventsResult, exceptionsResult, recoveryTasksResult] = await Promise.all([
    ...
    client.from(TABLES.RECOVERY_TASKS).select('status').eq('merchant_id', merchantId).limit(10000),
  ]);
  ...
  if (recoveryTasksResult.error) throw new Error(`automation_metrics_recovery_tasks_failed: ${recoveryTasksResult.error.message}`);
  ...
  return {
    ...
    recoveryTasks: (recoveryTasksResult.data ?? []).filter((task) => task.status !== 'completed').length,
  };
}
