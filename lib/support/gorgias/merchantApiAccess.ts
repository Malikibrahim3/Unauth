...
export type GorgiasMerchantApiAccess = {
  providerBaseUrl: string;
  credentials: GorgiasApiCredentials;
};

export async function getActiveGorgiasMerchantApiAccess(
  supabase: unknown,
  merchantId: string
): Promise<GorgiasMerchantApiAccess | null> {
  ...
  return {
    ...
    credentials: decryptGorgiasApiCredentials(data.access_token_encrypted),
  };
}
