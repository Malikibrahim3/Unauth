...
const reportDir = path.join(process.cwd(), 'reports/ux-audit')
const evidencePath = path.join(reportDir, 'ux-audit-evidence.json')
const evidence = fs.existsSync(evidencePath)
  ? JSON.parse(fs.readFileSync(evidencePath, 'utf8'))
  : { generatedAt: new Date().toISOString(), routes: [], pagesVisited: [], clicks: [], inventories: [], limitations: ['Playwright evidence file was not generated.'] }

const s = (name) => `![${name}](screenshots/${name}.png)`
const clickedRows = (evidence.clicks ?? [])
  .map((c) => `| ${c.page} | ${c.label} | ${c.outcome.replace(/\|/g, '\\|')} | ${c.patternRecommendation ?? 'n/a'} |`)
  .join('\n')

const routeList = (evidence.routes ?? []).map((route) => `- \`${route}\``).join('\n')
const hasUploadLimitation = (evidence.limitations ?? []).some((item) => item.includes('Upload processing did not reach results'))
const limitationList = (evidence.limitations ?? []).length
...
${s('audit-results-transactions-tab')}
...
|---|---|---|---|
${clickedRows || '| n/a | n/a | No clicks were recorded. | n/a |'}
...
fs.writeFileSync(path.join(reportDir, 'UX_AUDIT_REPORT.md'), report)
console.log(`Wrote ${path.join(reportDir, 'UX_AUDIT_REPORT.md')}`)
