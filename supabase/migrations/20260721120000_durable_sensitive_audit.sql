...
create index if not exists domain_event_deliveries_expired_lease_idx
  on public.domain_event_deliveries(handler_name, leased_until)
  where status = 'processing';

-- A worker can disappear after claiming a delivery. Reclaim expired leases,
-- while moving a final expired attempt to the bounded dead-letter state rather
-- than leaving it stuck in `processing` forever.
