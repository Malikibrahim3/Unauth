...
create index idx_claims_order on claims(merchant_id, source_order_id)
  where source_order_id is not null;
create index idx_claims_merchant_status on claims(merchant_id, status);
create index idx_claims_identity on claims(identity_id) where identity_id is not null;
...
declare t text;
begin
  foreach t in array array['claims','claim_events','claim_outcomes','claim_evidence'] loop
    execute format('alter table %I enable row level security', t);
  end loop;
end $$;
create policy claims_member_select on claims for select to authenticated using (is_merchant_member(merchant_id));
create policy claims_member_update on claims for update to authenticated using (is_merchant_member(merchant_id));
...
create policy claim_outcomes_member_select on claim_outcomes for select to authenticated
  using (exists (select 1 from claims c where c.id = claim_outcomes.claim_id and is_merchant_member(c.merchant_id)));
