...
describe('audit trail claim events', () => {
  beforeEach(() => {
    jest.resetAllMocks();
    (createClient as jest.Mock).mockReturnValue({ auth: { getUser: jest.fn().mockResolvedValue({ data: { user: { id: 'user-1' } } }) } });
    (createScopedClient as jest.Mock).mockImplementation((_merchantId: string, service: any) => service);
    (requirePermission as jest.Mock).mockResolvedValue({ denied: null, ctx: { merchantId: 'm-1', userId: 'user-1', role: 'owner' } });
    setupAuditService();
  });

  it('returns merchant-scoped claim events in audit trail response', async () => {
    const res = await GET(new NextRequest('http://localhost/api/audit-trail?resourceType=claim'));
    const body = await res.json();

    expect(res.status).toBe(200);
    expect(body.rows).toEqual(expect.arrayContaining([expect.objectContaining({
