...
describe('app route registry', () => {
  ...
  it('generates sidebar, workbench, and command palette from registry', () => {
    const sidebarHrefs = getSidebarNavItems().flatMap((g) => g.items.map((i) => i.href));
    ...
    for (const href of [...sidebarHrefs, ...workbenchHrefs, ...paletteHrefs]) {
      const route = Object.values(APP_ROUTES).find((r) => r.href === href.split('?')[0]);
      expect(route ?? href.startsWith('/customers?')).toBeTruthy();
    }
  });

  it('matches snapshot of sidebar labels', () => {
    const labels = getSidebarNavItems().flatMap((g) => g.items.map((i) => i.label));
