...
describe('claim SLA helpers', () => {
  ...
  it('marks open claims older than 168h (a week) as overdue', () => {
    expect(getClaimSlaState({ status: 'open', submitted_at: '2026-05-20T11:59:00.000Z' }, now).state).toBe('overdue');
  });

  it('marks 120-168h claims as approaching SLA', () => {
    expect(getClaimSlaState({ status: 'open', submitted_at: '2026-05-22T08:00:00.000Z' }, now).state).toBe('approaching');
  });
