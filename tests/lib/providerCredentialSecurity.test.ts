...
describe('provider credential security', () => {
  ...
  it('never falls back to a global credential in production', async () => {
    ...
    await expect(getProviderCredential('merchant-1', 'shipbob', {} as never)).resolves.toBeNull();
  });
  it('never falls back to a global credential in development or test', async () => {
