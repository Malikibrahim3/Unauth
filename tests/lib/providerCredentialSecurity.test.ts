...
describe('provider credential security', () => {
  ...
  it('never falls back to a global credential in production', async () => {
    ...
    await expect(getProviderCredential('merchant-1', 'shipbob', 'TEST_PROVIDER_KEY', {} as never)).resolves.toBeNull();
  });
  it('allows a development-only fallback', async () => {
    ...
    await expect(getProviderCredential('merchant-1', 'shipbob', 'TEST_PROVIDER_KEY', {} as never)).resolves.toBe('development-key');
  });
});
