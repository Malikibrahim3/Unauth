...
describe('payout-control reports contract', () => {
  it('uses payout and recovery labels instead of store/network intelligence copy', () => {
    const view = read('app/(app)/reports/ReportsPageView.tsx');
    const overview = read('app/(app)/reports/ReportsOverviewTab.tsx');
    const recovery = read('app/(app)/reports/ReportsLiveTab.tsx');

    const combined = `${view}\n${overview}\n${recovery}`;
    expect(combined).toContain('Payout reports');
    expect(combined).toContain('Payout exposure reviewed');
    expect(combined).toContain('Payouts denied under policy');
    expect(combined).toContain('Evidence gap trends');
    expect(combined).toContain('Recovery win rate');
    expect(combined).toContain('Partner performance');
  ...
  it('rolls payout exposure and requested actions into report breakdowns', () => {
    ...
    expect(buildClaimTypeBreakdown(claims)[0]).toMatchObject({ label: 'Item not received', value: 100 });
    expect(buildRequestedActionBreakdown(claims).map((row) => row.label)).toEqual(['Refund', 'Replacement']);
  });
